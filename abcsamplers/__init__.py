...
#     def sample(
#             self, 
#             n_samples:int,  # number of accepted samples per SMC epoch
#             thresholds:Iterable[float],  # abc error thresholds for every SMC epoch
#             iter_max:int=None  # max iterations per SMC epoch despite acceptance rate
#         ):
#         for n_epoch, threshold in enumerate(thresholds):
#             print(f"Sampling in sequence #{n_epoch + 1} with e={threshold}")
            
#             # initialize epoch
#             n_iter = 0
#             n_accepted = 0
#             accepted_samples = {name:[] for name in self.priors[n_epoch].keys()}
            
#             while n_accepted < n_samples or n_iter == iter_max:
#                 # sample from current priors
#                 samples = self.draw(self.priors[n_epoch])
#                 # simulate
#                 sim = self.simulator(samples)
#                 # summary statistics
#                 s = self.summary(sim)
#                 # distance
#                 d = self.distance(s, self.observed)
#                 # acceptance criteria
#                 if d < threshold:
#                     n_accepted += 1
#                     self.store_accepted_samples(
